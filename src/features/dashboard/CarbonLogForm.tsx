...
interface CarbonLogFormProps {
  ...
  /** Optional preselected date string (defaults to today) */
  defaultDate?: string;
}

interface CommuteFieldsProps {
  register: UseFormRegister<DailyLogFormData>;
  errors: FieldErrors<DailyLogFormData>;
}
...
interface EnergyFieldsProps {
  register: UseFormRegister<DailyLogFormData>;
  errors: FieldErrors<DailyLogFormData>;
  selectedHeatingSource: HeatingSource;
}

/**
 * Sub-component for transport commute inputs
 */
const CommuteFields: React.FC<CommuteFieldsProps> = ({ register, errors }) => (
...
/**
 * Sub-component for daily diet inputs
 */
const DietFields: React.FC<DietFieldsProps> = ({ register }) => (
  <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
    <div className="flex items-center gap-2 text-emerald-800 font-extrabold mb-1 text-xs uppercase tracking-wider">
...
/**
 * Sub-component for household energy inputs
 */
const EnergyFields: React.FC<EnergyFieldsProps> = ({
  register,
  errors,
  selectedHeatingSource
}) => (
  <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
    <div className="flex items-center gap-2 text-emerald-800 font-extrabold mb-1 text-xs uppercase tracking-wider">
...
/**
 * CarbonLogForm Component
 * Renders structured daily carbon logs input form mapped via react-hook-form and zod schema validations.
 */
  ...
  return (
    <Card className="border-slate-200 bg-white h-full shadow-lg">
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 text-sm">
        ...
        {/* Modular Fields Rendering */}
        <CommuteFields register={register} errors={errors} />
        <DietFields register={register} />
        <EnergyFields register={register} errors={errors} selectedHeatingSource={selectedHeatingSource} />
